...
결론: 공유망의 무자격 명령 쓰기를 재현하고 평가자 네트워크를 분리했다. 이름 기반 접속 실패는 확인했으나 IP 직접 차단과 조치 후 패킷 기반 정상 동작 재검증은 보완 중이다.

문서 정정: 2026-09-19 리뷰를 반영했다. 원본 증적은 변경하지 않았으며 새 실험 성공을 추가하지 않았다. 시험 항목별 상태는 [시험 결과표](../docs/test-matrix.md), 보완 절차는 [재검증 실행서](../docs/retest-runbook.md)를 기준으로 한다.
...
평가 도구를 별도 내부 `assessment` 네트워크로 이동하고 OpenPLC·FUXA는 기존 `control` 네트워크에 유지했다. 동일 평가 도구는 이름 해석에 실패했다. FUXA ON·OFF와 값 갱신은 운영자가 성공을 보고했으나 조치 후 패킷은 없다. 발견사항 AC-01은 이 실습의 신뢰 모델에서 **높음**으로 평가했으며, 상태는 **분리 구성 적용, 재검증 부분 완료**다.
...
| 태그 | PLC 변수 | FUXA 표시 | Wire 주소 | 자료형·HMI 사용 목적 |
| --- | --- | --- | --- | --- |
...
표의 읽기는 HMI의 사용 목적이다. 서버가 해당 주소의 쓰기를 거부한다는 권한 검증 결과가 아니다.
...
6. 평가자 네트워크만 분리하고 동일 도구의 이름 기반 접속 실패를 확인했다.
7. 분리 후 FUXA 동작에 대한 운영자 보고와 화면을 수집했다. 독립적인 조치 후 패킷 재검증은 남아 있다.
...
현재 internal: true는 assessment에만 적용되며 control에는 적용되지 않았다. 제어망 외부 통신 제한은 별도 후속 검토 사항이다. 이 조치는 Docker 네트워크 연결을 client allowlist처럼 사용한다. Modbus 기능코드별 필터링, endpoint 인증 또는 암호화를 추가한 것은 아니다.
...
| 재검증 조건 | 결과 |
| --- | --- |
| 이름 기반 접속 | 관찰 확인 — 이름 해석 실패, Modbus 단계 미도달 |
| IP 직접 TCP 5020 차단 | 미검증 — DNS 실패로 대체 판정하지 않음 |
| PLC 서비스 유지 | 부분 확인 — 운영자 보고; 미재생성 사실만으로 가용성을 판정하지 않음 |
| 허용된 FUXA 동작 유지 | 사용자 관찰 PASS — 값 갱신과 ON·OFF 동작 확인 |
| 종료 상태 복원 | 화면상 OFF 확인 — 독립적인 조치 후 태그 읽기 미확보 |
...
후속 우선순위는 IP 직접 경로와 허용 HMI 통신 재검증, 실행 버전 식별, 새 PC 복원 시험, HMI 명칭 개선이다. 현재 확인한 digest를 과거 시험 버전으로 소급하지 않는다. 제품이 지원하고 운영 요구가 맞는 경우 protocol-aware 정책이나 인증된 보안 전송을 별도 방어 계층으로 검토한다.

## 11. 최종 판정

이 프로젝트는 범위 설정, PLC/HMI 구축, 정상 패킷 해석, 제한된 접근통제 검증, 원인과 위험 설명, 네트워크 분리 구성까지 수행했다. 네트워크 차단과 조치 후 정상 운영 유지의 독립적 재검증은 부분 완료이며 Finding을 완전히 종결하지 않는다. 결과는 실제 생산환경의 안전성이나 표준 준수를 증명하지 않지만, OT 보안 진단에서 필요한 증거 중심 판단과 변경 후 운영 검증 과정을 재현한다.
